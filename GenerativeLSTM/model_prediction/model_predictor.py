...
class ModelPredictor():
    ...
    def __init__(self, parms, input_folder=os.path.join('GenerativeLSTM','input_files') , output_folder=os.path.join('GenerativeLSTM','output_files'), rules_path=os.path.join('GenerativeLSTM','rules.ini'), root_path=None):
        ...
        Args:
            ...
            root_path: Project root path (reserved for future use).
        """
    ...
    def execute_predictive_task(self):
        ...
        if self.parms['activity'] == 'pred_log':
            test_log_cases = len(self.log.caseid.unique())
            target = self.parms.get('num_instances', test_log_cases)
            self.parms['len_log'] = target
            self.parms['num_cases'] = target
    ...
    def export_predictions(self, r_num):
        ...
        # Cuando no se generó ningún trace que cumpla las condiciones, la función
        # devuelve [] en vez de un DataFrame — convertir a DataFrame vacío.
        if not isinstance(df_traces_generated, pd.DataFrame):
            df_traces_generated = pd.DataFrame(columns=cols)
        ...
        # Validacion defensiva (agregada 2026-08-12): si ningun intento de
        # generacion produjo una traza que cumpliera la regla impuesta,
        # `final_log` queda vacio -- antes esto se escribia igual a CSV y fallaba
        # mas adelante con un KeyError('caseid') critico dentro de
        # readers.LogReader al leer un log sin filas. No cambia ningun
        # comportamiento del generador (no toca como se generan/filtran las
        # trazas), solo falla mas rapido y con un mensaje que explica que paso.
        if final_log.empty:
            rule_info = self.parms.get('rules', {})
            raise RuntimeError(
                "La alucinacion no produjo ninguna traza que cumpla la regla impuesta "
                f"(path={rule_info.get('path')!r}, rule={rule_info.get('rule')!r}). "
                f"0 trazas validas de {len(files_gen)} intento(s) de generacion "
                f"(carpeta: {self.parms.get('traces_gen_path')!r}). Esto suele pasar "
                "cuando el modelo esta poco entrenado (ej. muy pocos epochs) y no logra "
                "navegar la restriccion durante la generacion autoregresiva restringida, "
                "o cuando la regla es demasiado restrictiva para este proceso. Revisar "
                "antes de continuar."
            )
