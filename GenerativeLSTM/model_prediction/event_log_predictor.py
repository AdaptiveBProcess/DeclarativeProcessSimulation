        ...
        def gen(cases, parms, model_path, vectorizer):
            ...
            try:
                ...
                for case in cases:
                    ...
                    if parms['variant'] not in ('Rules Based Random Choice', 'Rules Based Arg Max'):
                        # No rule to enforce under this variant -- accept every generated
                        # trace unconditionally instead of filtering by rule compliance.
                        # Without this branch, the rule-acceptance logic below ran
                        # regardless of `variant` and silently biased "unconstrained" runs
                        # (e.g. the Placebo baseline, which reuses another arm's rule.ini
                        # only to satisfy extract_rules) toward whatever rule happened to
                        # be loaded (CLAUDE.md 27).
                        ...
                        if n_files_gen + 1 >= parms['len_log']:
                            flag = False
                            break
                        continue
                    ...
                    #The trace that fulfill the rule is combined with all traces
                    # <= / >= instead of strict < / > : with strict inequalities, current_prop
                    # lands exactly on new_prop_cases (guaranteed after the 1st accepted trace when
                    # new_prop_cases==1.0) and neither branch can ever fire again, permanently
                    # starving generation regardless of remaining budget.
